...
void ofApp::draw(){
	...
	for (int i = 0; i < NUM_CIRCLES; i++) {
        ...
        // FIXME: This won't draw for just one of the circles, but I'm not sure why.
		...
		for (int j = 0; j < NUM_POINTS; j++) {
			...
			for (int k = 0; k < lineLength; k++) {
				...
				// FIXME: These angle offsets are supposed to be tiny, but they keep on somehow being huge.
	...
	// // To test, just draw one circle of that radius for now
	// ofSetColor(255);
	// ofNoFill();
	// ofDrawCircle(xCenterMain, yCenterMain, minSwarmRadius);
...
//			drawDot(x, y); // , R_DOT, 255, 255, 255);
            drawDot(x, y, R_DOT, 1.25);
            if (i < numSwarmCircles / 2) {
//                drawDot(x, y, 1, 255, 255, 255, ofMap(i, 0, numSwarmCircles / 2, 0.5, 0.1) * 255);
